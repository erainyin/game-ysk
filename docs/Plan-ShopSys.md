...
- 玩家点击卡牌后先弹出确认操作层，点击“确认购买”后才扣除对应点数并加入手牌；点击“我再想想”关闭确认层。
...
`selectShopCard()` 调用现有的 `purchaseCard()` 完成扣点和加牌，`sellShopCard()` 通过卡牌实例移除手牌并返还 `Math.max(1, Math.floor(cost / 3))` 点数。出售不会结束商店回合，玩家可以继续购买；`cancelShop()` 只清理商店状态。购买和关闭都会通知状态更新并调用 `nextTurn()`。
