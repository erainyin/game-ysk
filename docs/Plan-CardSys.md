...
| `retreat_all` | 自身牺牲 n 点血量，并让所有其他存活玩家后退 3 格 | `steps`（默认 -3），`sacrifice` 由场上存活玩家数量决定 | 即时 |
...
#### 卡牌7：大撤退 (great_retreat)

- **名称**：大撤退
- **类型**：攻击型
- **描述**：所有其他存活玩家倒退 3 格，并牺牲自身 n 点血量，n 为当前场上存活玩家数量
- **图标**：`great_retreat.png`
- **花费**：5 点
- **目标**：`self`（自身，且不需要指定目标）
- **效果**：
  ```javascript
  [{ type: 'retreat_all', params: { steps: -3 } }]
  ```
- **规则说明**：
  - 计算 `n` 时，统计当前场上未死亡、未胜利的玩家数量，包括自身
  - 受影响的目标仅为除自身以外的存活玩家
  - 反弹/护盾不生效于此“自我牺牲”伤害，属于直接代价；若血量归零则走常规死亡判定与不死状态检测
  - 该卡为强力大招，适合在局面即将失控时作为反制手段
